            ...
            grid.innerHTML = musiques.map(m => `
                <div class="spotify-card p-4 rounded-lg cursor-pointer group">
                    <div class="relative mb-4">
                        <img src="https://via.placeholder.com/300" class="w-full aspect-square object-cover rounded-md shadow-lg">
                    ...
                    <a href="/musiques/${m.id}" class="font-bold block truncate hover:underline">${m.titre}</a>
                    <div class="text-sm text-gray-400 mt-1">
                        ${m.artistes.map(a => `<a href="/artistes/${a.id}" class="hover:underline">${a.nom}</a>`).join(', ')}
                    </div>
                </div>
            `).join('');
        });
